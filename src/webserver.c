...
static esp_err_t config_io_names_handler(httpd_req_t *req)
{
    ...
    n = snprintf(p, remaining,
        ...
        "<button type=\"button\" class=\"danger\" style=\"margin-left:10px;\" onclick=\"saveAndReboot()\">Save &amp; Reboot</button>"
        "</form>"
        "<script>"
        "function saveAndReboot(){"
        "var form=document.querySelector('form[action=\"/config/io\"]');"
        "var fd=new FormData(form);"
        "var xhr=new XMLHttpRequest();"
        "xhr.open('POST','/config/io',true);"
        "xhr.onload=function(){"
        "if(xhr.status==200){"
        "var r=new XMLHttpRequest();"
        "r.open('POST','/reboot',true);"
        "r.onload=function(){document.body.innerHTML=r.responseText;};"
        "r.send();"
        "}else{alert('Failed to save settings');}"
        "};"
        "xhr.onerror=function(){alert('Failed to save settings');};"
